...
import { sallata } from '../menu/sallata';
...
const Sallata = () => {
  ...
  const { ref: sectionRef, inView } = useInView({
    ...
    threshold: 0.1, // Percentage of the element that should be visible before triggering
  });
  ...
  return (
    <div ref={sectionRef} className='px-5 lg:px-20 2xl:px-56 mt-16 lg:mt-24 relative items-center w-full h-fit lg:pb-10'>
      ...
      {sallata?.map((item, index) => {
        const subItemChunks = chunkArray(item.subItems, 3);
        return (
          <div
            key={item.id}
            className='border-b border-[#11141C] lg:flex lg:items-center py-6 lg:py-6 lg:h-fit lg:relative uppercase'
            ref={addToRefs}
          >
            <Link to={item.link} className='text-[#11141C] font-custom text-[19px]'>
              {item.category}
            </Link>
            <div className='text-8xl pt-6 lg:text-[150px] font-custom1 text-[#8E2209] lg:ml-60 2xl:ml-72'>
              <p>{item.title}</p>
            </div>
